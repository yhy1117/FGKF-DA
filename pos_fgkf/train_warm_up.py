...
from model_warm_up import Model
import load_data
import embed
...
tf.flags.DEFINE_integer("num_classes", 18, "num_classes")

# model names
tf.flags.DEFINE_string("model_name", "pos_"+"weibo_capsule_warm_up", "model name")
...
tf.flags.DEFINE_integer("warm_up_epochs", 2, "Number of warm up epochs (default: 3)")
            ...
            for batch in batches_t:
                ...
                if current_step % FLAGS.evaluate_every == 0:
                    ...
                    for batch in batches_s:
                        ...
                        x_batch = np.asarray(x_batch, dtype=np.float32)
                        pred_batch = np.reshape(pred_batch, [FLAGS.batch_size, -1, 18])
