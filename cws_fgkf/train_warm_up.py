...
from config import DROP_SINGLE, BI_DIRECTION, BI_GRAM, STACK_STATUS, LSTM_NET, TEST_PRED_TAG, dense_hidden

from model_three_warm_up import Model
import load_data
import embed
import build_bi_vocab
import data_helpers
import get_pred_p
...
# model names
tf.flags.DEFINE_string("model_name", "cws_"+"zx_warm_up_5", "model name")
...
tf.flags.DEFINE_integer("warm_up_epochs", 5, "Number of warm up epochs (default: 3)")
...
train_char_s, train_label_s = load_data.load_data_base(config.TRAIN_DATA_UNI)
...
dev_char_s, dev_label_s = load_data.load_data_base(config.DEV_DATA_UNI)
dev_pred = get_pred_p.load_datas(config.DEV_DATA_UNI_PRED)
#max_dev_sentene_length_s = max([len(x) for x in dev_char_s])
dev_x_s, dev_label_s, dev_pred = build_bi_vocab.build_bi_vocabulary(dev_char_s, dev_label_s, dev_pred, max_sentene_length, BI_GRAM)

#load test data
print('loading test data...')
test_char_s, test_label_s = load_data.load_data_base(config.TEST_DATA_UNI)
test_pred = get_pred_p.load_datas(config.TEST_DATA_UNI_PRED)
#max_test_sentene_length = max([len(x) for x in test_char])
test_x_s, test_label_s, test_pred = build_bi_vocab.build_bi_vocabulary(test_char_s, test_label_s, test_pred, max_sentene_length, BI_GRAM)

print('load teacher data...')
print('loading train data...')
train_char_t, train_label_t = load_data.load_data_base(config.T_TRAIN_DATA_UNI)

# Build vocabulary
print("Build vocabulary...")
max_sentene_length = config.MAX_LEN
train_x_t, train_label_t = build_bi_vocab.build_bi_vocabulary_t(train_char_t, train_label_t, max_sentene_length, BI_GRAM)

#load dev data
print('loading dev data...')
dev_char_t, dev_label_t = load_data.load_data_base(config.T_DEV_DATA_UNI)
#max_dev_sentene_length_s = max([len(x) for x in dev_char_s])
dev_x_t, dev_label_t = build_bi_vocab.build_bi_vocabulary_t(dev_char_t, dev_label_t, max_sentene_length, BI_GRAM)

#load test data
print('loading test data...')
test_char_t, test_label_t = load_data.load_data_base(config.T_TEST_DATA_UNI)
#max_test_sentene_length = max([len(x) for x in test_char])
test_x_t, test_label_t = build_bi_vocab.build_bi_vocabulary_t(test_char_t, test_label_t, max_sentene_length, BI_GRAM)
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def train_step_student(type, index_batch, x_batch, y_batch, y_pred_batch, y_class_batch, seq_len_batch):
            if FLAGS.model_type == 'type I':
                beta, step, loss, loss_ref, loss_adv, acc = model.train_step_student(sess, type,  index_batch, x_batch, y_batch, y_pred_batch, y_class_batch, seq_len_batch, FLAGS.dropout_keep_prob)
                betas.append(beta)
            elif FLAGS.model_type == 'type II':
                alpha, step, loss, loss_ref, loss_adv, acc = model.train_step_student(sess, type,  index_batch, x_batch, y_batch, y_pred_batch, y_class_batch, seq_len_batch, FLAGS.dropout_keep_prob)
                alphas.append(alpha)
            elif FLAGS.model_type == 'type III':
                alpha, beta, step, loss, loss_ref, loss_adv, acc = model.train_step_student(sess, type, index_batch, x_batch,
                                                                               y_batch, y_pred_batch, y_class_batch,
                                                                               seq_len_batch, FLAGS.dropout_keep_prob)
                alphas.append(alpha)
                betas.append(beta)
        ...
        def train_step_student_warm(x_batch, y_batch, seq_len_batch):
            step, loss = model.train_step_student_warm(sess, x_batch, y_batch, seq_len_batch, FLAGS.dropout_keep_prob)

            time_str = datetime.datetime.now().isoformat()
            print("S: {}: step {}, loss {:g}".format(time_str, step, loss))

            return step

        def train_step_teacher_warm(x_batch, y_batch, seq_len_batch):
            step, loss = model.train_step_teacher_warm(sess, x_batch, y_batch, seq_len_batch, FLAGS.dropout_keep_prob)

            time_str = datetime.datetime.now().isoformat()
            print("T: {}: step {}, loss {:g}".format(time_str, step, loss))
        ...
        def final_test_step_student(x, label, pred, epoch, test=False, bigram=False):
            ...
            if test:
                ...
                loss_dev, y_true, y_pred = model.fast_all_predict_sdudent_dev(sess, N, x, label, pred, epoch)
        ...
        for j in range(FLAGS.warm_up_epochs):
            print('warming up...')
            batch_w = data_helpers.batch_iter(list(zip(train_x_t, train_label_t)), FLAGS.batch_size, 1)
            for batch in batch_w:
                x_batch, y_batch = zip(*batch)
                seq_len_batch = [int(len(x) / 5) for x in x_batch]
                x_batch = np.asarray(x_batch, dtype=np.float32)
                current_step_t = train_step_teacher_warm(x_batch, y_batch, seq_len_batch)
                current_step_s = train_step_student_warm(x_batch, y_batch, seq_len_batch)
        ...
        if FLAGS.model_type == 'type I' or FLAGS.model_type == 'type III':
            f_1 = open(config.T_TEST_SENT_P, 'w')
            for i in range(len(betas)):
                for j in range(len(betas[i])):
                    f_1.write(str(betas[i][j]))
                    f_1.write('\n')
                f_1.write('\n')
            f_1.close()

        if FLAGS.model_type == 'type II' or FLAGS.model_type == 'type III':
            f_2 = open(config.ATTENTION_FILE, 'w')
            for i in range(len(alphas)):
                for j in range(len(alphas[i])):
                    for k in range(len(alphas[i][j])):
                        f_2.write(str(alphas[i][j][k]))
                        f_2.write('\n')
                    f_2.write('*')
                f_2.write('***')
            f_2.close()

        f_3 = open(config.DEV_LOSS_FILE, 'w')
        for m in range(len(dev_losses)):
            f_3.write(str(dev_losses[m]))
            f_3.write('\n')
        f_3.close()
