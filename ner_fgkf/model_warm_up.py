...
class Model(object):
    def __init__(self, batch_size, vocab_size, word_dim, lstm_dim, num_classes,
        ...
        with tf.variable_scope("train_student_ops"):
            ...
            grads_warm, __ = tf.clip_by_global_norm(tf.gradients(self.loss_seg_student_dev, tvars), self.clip)#对梯度进行截取
            self.train_op_s_warm = self.optimizer_s.apply_gradients(zip(grads_warm, tvars), global_step=self.global_step_s)#将算出的梯度应用到变量上
    ...
    def train_step_student_warm(self, sess, x_batch, y_batch, seq_len_batch, dropout_keep_prob):
        ...
        _, step, loss = sess.run([self.train_op_s_warm, self.global_step_s, self.loss_seg_student_dev], feed_dict)

        return step, loss

    def train_step_teacher_warm(self, sess, x_batch, y_batch, seq_len_batch, dropout_keep_prob):
