...
class Model(object):
    def __init__(self, batch_size, vocab_size, word_dim, lstm_dim, num_classes,
                 ...
                 lstm_net, bi_direction, alpha, alpha_decay, out_caps_num, rout_iter, rout_mode, dense_hidden,
                 attn_mode, model_type):
        ...
        def domain_layer(output, seq_len):
            W_classifier = tf.get_variable(shape=[2 * lstm_dim, 2], initializer=tf.truncated_normal_initializer(
                stddev=1.0 / math.sqrt(float(2))), name='W_classifier')
        ...
        with tf.variable_scope("attention"):
            ...
            if self.attn_mode == 'dot':
                ...
                for i in range(batch_size):
                    attn.append(tf.tensordot(self.v, tf.nn.tanh(
                        tf.add(tf.add(tf.matmul(self.output_student[i], self.W_2), student_sent_rep_w[i]), self.b_a)),
                                             axes=[[0], [1]]))
                    # sent_a = []
        ...
        with tf.variable_scope("loss_seg_student_params"):
            ...
            self.weighted_unary_scores = tf.concat(axis=2, values=[weighted_unary_scores_0, weighted_unary_scores_1,
                                                                   ...
                                                                   weighted_unary_scores_6])

            log_likelihood_seg_s, self.transition_params_s = tf.contrib.crf.crf_log_likelihood(
                self.weighted_unary_scores,
                self.y, self.seq_len)
        ...
        with tf.variable_scope("loss_ref"):
            ...
            if self.model_type == 'type II':
                ...
                self.beta = tf.exp(np.multiply(np.add(self.tao, tf.multiply(sent_p_true, -1)), self.gammma))
                # self.beta = tf.exp(np.multiply(np.add(sent_p_true, -1), self.gammma))
                w_pred_p = []
                for j in range(batch_size):
        ...
        with tf.variable_scope("loss_teacher"):
            log_likelihood_seg_t, self.transition_params_t = tf.contrib.crf.crf_log_likelihood(self.unary_scores_t,
                                                                                               self.y, self.seq_len)
        ...
        with tf.variable_scope("train_student_ops"):
            ...
            self.train_op_s = self.optimizer_s.apply_gradients(zip(grads, tvars),
                                                               global_step=self.global_step_s)  # 将算出的梯度应用到变量上
        ...
        with tf.variable_scope("train_teacher_ops"):
            ...
            self.train_op_t = self.optimizer_t.apply_gradients(zip(grads, tvars),
                                                               global_step=self.global_step_t)  # 将算出的梯度应用到变量上
            self.domain_op_t, self.global_step_domain_t = trainingDomain(self.D_loss_teacher)

    def train_step_student(self, sess, type, index_batch, x_batch, y_batch, y_pred_batch, y_class_batch, seq_len_batch,
                           dropout_keep_prob, is_train=True):
        ...
        if type == 'type II' or type == 'type III':
            loss_ref, attn, alphas, _, step, loss, __, step_adv, loss_adv, acc = sess.run(
                [self.loss_ref, self.attn_normed, self.alpha_new, self.train_op_s, self.global_step_s,
                 self.loss_student, self.domain_op_s, self.global_step_domian_s, self.D_loss_student, self.accuracy_s],
                feed_dict)
            ...
            loss_ref, _, step, loss, __, step_adv, loss_adv, acc = sess.run(
                [self.loss_ref, self.train_op_s, self.global_step_s, self.loss_student, self.domain_op_s,
                 self.global_step_domian_s, self.D_loss_student, self.accuracy_s], feed_dict)
        return step, loss, loss_ref, loss_adv, acc

    def train_step_student_seg(self, sess, x_batch, y_batch, y_pred_batch, seq_len_batch, dropout_keep_prob,
                               is_train=True):
    ...
    def train_step_student_d(self, sess, x_batch, y_class_batch, seq_len_batch, dropout_keep_prob, is_train=True):
        ...
        _, step_adv, loss_adv, acc = sess.run(
            [self.domain_op_s, self.global_step_domian_s, self.D_loss_student, self.accuracy_s], feed_dict)

        return step_adv, loss_adv, acc

    def train_step_teacher(self, sess, x_batch, y_batch, y_class_batch, seq_len_batch, dropout_keep_prob,
                           is_train=True):
        ...
        _, step, loss, __, step_adv, loss_adv, acc = sess.run(
            [self.train_op_t, self.global_step_t, self.loss_teacher, self.domain_op_t, self.global_step_domain_t,
             self.D_loss_teacher, self.accuracy_t], feed_dict)
    ...
    def train_step_teacher_d(self, sess, x_batch, y_class_batch, seq_len_batch, dropout_keep_prob, is_train=True):
        ...
        _, step_adv, loss_adv, acc = sess.run(
            [self.domain_op_t, self.global_step_domain_t, self.D_loss_teacher, self.accuracy_t], feed_dict)
    ...
    def fast_all_predict_sdudent(self, sess, N, x, label, pred, epoch, is_train=False):
        ...
        for batch in batches:
            ...
            y_pred_batch = np.reshape(y_pred_batch, [-1, 100, self.num_classes])
            # infer predictions
            feed_dict = {
                self.x: x_batch,
                self.y: y_batch,
    ...
    def fast_all_predict_p_teacher(self, sess, N, x, label, epoch, bigram, is_train=False):
        ...
        for batch in batches:
            ...
            logits, unary_scores, transition_params = sess.run(
                [self.sent_p, self.unary_scores_t, self.transition_params_t], feed_dict)
